...
flowchart LR
    ...
    Step2 --> Step3["3. APK 설치 완료 후 즉시 1회 실행!\n(권한 마법사 터치 ➔ 점 3개 생성 ⭐)"]
    Step3 --> Step4["4. 앱정보 ➔ [점 3개 ⋮] 터치\n([제한된 설정 허용] 지문 인증)"]
    Step4 --> Step5["5. [SMS/알림] 권한 허용 완료!\n(24시간 릴레이 정상 가동)"]
```
...
### 5단계: 어플 설치 후 무조건 1회 실행 ➔ '제한된 설정' 해제 (SMS 권한 활성화 ⭐⭐⭐)

> ⚠️ **필독 (무조건 어플을 1회 먼저 실행해야 [점 3개 ⋮]가 생깁니다!)**:  
> 안드로이드 보안 정책상, 앱을 설치하자마자 곧바로 단말기 설정으로 가면 **점 3개(⋮) 메뉴가 아예 보이지 않습니다.**  
> **반드시 설치된 `Hi_Dorm` 어플을 먼저 1회 켜서 [권한 설정 마법사]나 [허용하기]를 눌러야** 시스템에 차단 로그가 등록되면서 비로소 설정 화면에 **[점 3개(⋮)]**가 나타납니다!

```mermaid
flowchart LR
    R0["1. 어플 설치 즉시 1회 실행!\n(권한 요청/마법사 켜기 ➔ 점 3개 생성)"] --> R1["2. 앱 아이콘 1초간 꾹 누르기\n(ⓘ 앱 정보 터치)"]
    R1 --> R2["3. 우측 상단 [점 3개 ⋮] 터치\n(비로소 메뉴 생성 확인!)"]
    R2 --> R3["4. [제한된 설정 허용] 터치\n(지문/PIN 본인 인증)"]
    R3 --> R4["5. [권한] ➔ [SMS] 허용!\n(24시간 릴레이 정상 가동)"]
```

#### 📌 진행 순서 (10초 완결):
1. **어플 1회 실행:** APK 설치가 끝나면 **무조건 `Hi_Dorm` 어플을 먼저 1회 켭니다.**  
   *(앱 상단의 방패 🛡️ 아이콘이나 마법사를 켜고 [허용하기]를 누르면 "SMS 액세스 거부됨" 같은 시스템 알림이 뜨는데, 이는 시스템에 기록을 남기기 위한 정상이므로 그대로 둡니다.)*
2. **앱 정보로 이동:** 홈 화면으로 나와 **`Hi_Dorm` 앱 아이콘을 1초간 꾹 누른 뒤 [ⓘ 앱 정보]**를 터치합니다.  
   *(또는 단말기 **[설정] ➔ [애플리케이션] ➔ [Hi_Dorm SMS Relay]**)*
3. **새로 생긴 [점 3개] 터치:** 앱 정보 화면 오른쪽 맨 위 구석에 새로 생긴 **[점 3개(⋮)]** 메뉴 아이콘을 터치합니다.
4. **`[제한된 설정 허용]`**을 터치하고 평소 쓰시는 **지문이나 PIN/패턴**을 인증합니다.
5. **SMS 권한 허용:** 이제 바로 아래 **[권한]** 메뉴로 들어가서 비활성화되어 있던 **[SMS]**와 **[알림]**을 모두 **`[허용]`**으로 켜주면 모든 준비가 완료됩니다!
