...
flowchart LR
    ...
    Step2 --> Step3["3. APK 터치 즉시 설치 완료!\n(출처 불명 앱 허용)"]
    Step3 --> Step4["4. 구글 '제한된 설정' 해제\n(앱정보 ➔ 점3개⋮ ➔ 제한 해제)"]
    Step4 --> Step5["5. SMS 권한 허용 완료!\n(24시간 릴레이 정상 가동)"]
```
...
### 5단계: 구글 「제한된 설정」 해제 (SMS 권한 회색 잠금 해제 ⭐⭐⭐)
안드로이드 13 및 14+부터는 구글 플레이스토어가 아닌 외부에서 설치한 앱의 **SMS 및 알림 권한을 "보안을 위해 이 설정은 현재 사용할 수 없습니다"라며 회색으로 잠그고 허용을 못하게 막습니다.** 아래 순서대로 진행하시면 즉시 잠금이 해제됩니다:

```mermaid
flowchart LR
    R1["앱 아이콘 길게 누르기\n(ⓘ 앱 정보 터치)"] --> R2["우측 상단 [점 3개 ⋮] 터치\n(앱 상세 화면)"]
    R2 --> R3["[제한된 설정 허용] 터치\n(지문/PIN 본인 인증)"]
    R3 --> R4["[권한] ➔ [SMS] 허용!\n(정상 발송 활성화)"]
```

1. 홈 화면에서 **`Hi_Dorm` 앱 아이콘을 길게 누른 뒤 [ⓘ 앱 정보]** 터치 (또는 **단말기 [설정] ➔ [애플리케이션] ➔ [Hi_Dorm SMS Relay]**)
2. 화면 오른쪽 상단의 **[점 3개(⋮)]** 메뉴 아이콘 터치
3. **[제한된 설정 허용 (Allow restricted settings)]** 선택
4. 본인 확인(지문, 얼굴 또는 PIN/패턴) 인증
5. 이제 해당 화면의 **[권한]** 메뉴로 들어가면, 비활성화되어 있던 **[SMS]** 및 **[알림]**을 정상적으로 **`허용`**할 수 있습니다!

> 💡 **팁:** 만약 앱 정보 화면에서 점 3개 메뉴에 '제한된 설정 허용'이 보이지 않는다면, 앱 내에서 [허용하기]를 눌러 차단 팝업을 1회 발생시킨 뒤 다시 들어가시면 메뉴가 즉시 표시됩니다. (또는 앱 내 [권한 설정 탭]의 [해제하러 가기] 버튼을 누르면 해당 화면으로 바로 이동합니다.)
