...
## 📱 갤럭시(Samsung Galaxy) 단말기 설치 방법 (⚠️ 설치 불가 시 필독)

최신 갤럭시 단말기(One UI 6.0 ~ 7.0+, 안드로이드 14+)는 **삼성 보안 엔진(Auto Blocker)**에 의해 외부 APK 설치가 기본적으로 차단됩니다. 아래 4단계를 순서대로 진행해 주세요:

### 1단계: 삼성 「보안 위험 자동 차단」 임시 끄기 (가장 중요 ⭐⭐⭐)
One UI 6+에 신설된 하드웨어 보안 기능으로, 켜져 있으면 모든 비인가 APK 설치가 원천 차단됩니다:
1. 단말기 **[설정]** 앱 실행
2. **[보안 및 개인정보 보호]** 메뉴 진입
3. **[보안 위험 자동 차단 (Auto Blocker)]** 터치
4. 상단의 스위치를 **`사용 안 함 (꺼짐)`**으로 **임시 비활성화**
> 💡 *설치가 완료된 후에는 해당 기능을 다시 켜도 앱 동작에 아무런 영향을 주지 않습니다.*

### 2단계: 「출처를 알 수 없는 앱 설치」 허용
1. **[설정] ➔ [보안 및 개인정보 보호] ➔ [출처를 알 수 없는 앱 설치]** 진입
2. APK를 다운로드한 브라우저(예: **Chrome**, **삼성 인터넷**) 또는 **내 파일** 앱의 토글을 **`허용`**으로 켭니다.

### 3단계: Google Play 프로텍트 차단 해제 (설치 강제 차단 해결 ⭐⭐)
본 앱은 학생 안내 및 기숙사 릴레이를 위해 `SEND_SMS`(문자 발송) 시스템 권한을 포함하므로, 구글 플레이가 아닌 웹에서 설치할 때 구글 플레이 프로텍트가 차단합니다:

* **방법 A (버튼이 보일 때):**  
  **"Play 프로텍트에 의해 차단됨"** 팝업 하단의 **`[세부정보 더보기]`**를 누른 뒤 **`[무시하고 설치 (안전하지 않음)]`**를 클릭합니다.
* **방법 B ("무시하고 설치"가 안 보이고 그냥 차단될 때 - 100% 해결):**  
  최신 안드로이드에서는 SMS 권한이 있는 외부 앱에 대해 "무시하고 설치" 버튼을 아예 숨겨버립니다. 이 경우 Play 프로텍트를 **10초간만 잠깐 꺼두고 설치**하시면 됩니다:
  1. 단말기에서 **`Google Play 스토어`** 앱 실행
  2. 우측 상단 **[프로필 아이콘]** 터치
  3. **[Play 프로텍트]** 메뉴 선택
  4. 우측 상단 **[톱니바퀴 (설정)]** 아이콘 터치
  5. **`Play 프로텍트로 앱 검사`** 스위치를 **`끄기 (비활성화)`**
  6. 이제 다운로드한 APK를 누르면 **아무런 차단 없이 1초 만에 정상 설치**됩니다!
  > 💡 *설치가 완료된 후에는 Play 스토어에서 해당 스위치를 다시 켜셔도 이미 설치된 앱은 정상 작동합니다.*

### 4단계: 기존 구버전 앱 충돌 방지 (서명 충돌 해결)
만약 이전에 설치했던 `Hi_Dorm` 테스트 앱이 폰에 이미 있다면 서명 불일치로 "앱이 설치되지 않았습니다"가 발생합니다:
