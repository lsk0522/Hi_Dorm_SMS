...
## 📱 단말기 설치 및 권한 설정 (모바일 1분 완성)

최신 안드로이드 및 삼성 갤럭시에서 보안 차단 없이 설치하고 SMS 권한을 켜는 **가장 빠른 5단계**입니다. 스마트폰을 보면서 순서대로 따라 해 주세요.

---

### 1. 삼성 보안 위험 자동 차단 끄기 (Galaxy 필수)
* **[설정] ➔ [보안 및 개인정보 보호] ➔ [보안 위험 자동 차단]**
* 상단 스위치를 **`사용 안 함 (꺼짐)`**으로 변경합니다.

---

### 2. Google Play 프로텍트 검사 끄기 ("무시하고 설치"가 없을 때)
* **[Play 스토어] 앱 실행 ➔ 우측 상단 [내 프로필] ➔ [Play 프로텍트]**
* 우측 상단 **`⚙️ (설정 톱니바퀴)` ➔ 맨 위 `Play 프로텍트로 앱 검사`** 스위치를 **`끄기`**로 변경합니다.
* *(※ 설치 완료 후 다시 켜셔도 이미 설치된 앱은 안전하게 유지됩니다.)*

---

### 3. APK 다운로드 및 설치
* [⬇️ 최신 app-release.apk 다운로드](https://github.com/lsk0522/Hi_Dorm_SMS/releases/download/v1.0.0/app-release.apk) 터치 후 설치를 진행합니다.
* *(기존에 설치된 구버전 앱이 있다면 먼저 길게 눌러 [설치 삭제] 후 새 버전을 설치하세요.)*

---

### 4. ⭐ 어플 무조건 1회 먼저 실행! (점 3개 생성 필수)
* 설치가 완료되면 **무조건 `Hi_Dorm` 어플을 먼저 1회 실행**합니다.
* 메인 상단의 방패(🛡️) 아이콘이나 권한 마법사를 한 번 켭니다.
* *(시스템에서 "SMS에 대한 액세스 거부됨"이 뜨는 것은 차단 기록을 남겨 점 3개를 만들기 위한 정상이니 그대로 두시면 됩니다.)*
* 이제 홈 화면으로 나옵니다.

---

### 5. ⭐ '제한된 설정' 해제하고 SMS 권한 켜기
1. 홈 화면에서 **`Hi_Dorm` 앱 아이콘을 1초간 꾹 누른 뒤 [ⓘ] (앱 정보)**를 터치합니다.  
   *(또는 [설정] ➔ [애플리케이션] ➔ [Hi_Dorm SMS Relay])*
2. 앱 상세 화면 오른쪽 맨 위 구석의 **[점 3개(⋮)]**를 터치합니다.
3. **`[제한된 설정 허용]`**을 터치하고 **지문이나 PIN/패턴**을 인증합니다.
4. 바로 아래 **[권한]** 메뉴로 들어갑니다.
5. **[SMS]**와 **[알림]**을 각각 터치하여 **`[허용]`**으로 켜주면 모든 설정이 끝납니다!
...
1. **배터리 최적화 완전 무제한:**
   `설정 > 애플리케이션 > Hi_Dorm SMS Relay > 배터리 > 제한 없음(Unrestricted)` (Doze 모드 해제)
2. **다른 앱 위에 표시 허용:**
   `설정 > 애플리케이션 > Hi_Dorm SMS Relay > 다른 앱 위에 표시 > 허용` (백그라운드 기동 우선순위 보장)
3. **백그라운드 사용 제한 해제:**
   `설정 > 배터리 > 백그라운드 앱 사용 제한 > 절전 예외 앱 추가`
4. **충전 중 화면 켜짐 유지 (Stay Awake):**
   `설정 > 개발자 옵션 > 충전 중 화면 켜짐 유지` 활성화 (화면 소등으로 인한 절전 차단)
5. **배터리 보호(스웰링 방지) 모드:**
   `설정 > 배터리 > 배터리 보호 (최대 80~85% 충전 제한)` (24시간 거치 시 배터리 부풀림 방지)
6. **SMS 및 알림 접근 권한 일괄 허용:**
   앱 최초 실행 시 요청되는 SMS 발송/수신, 알림 권한을 모두 허용합니다. (구글 제한된 설정 해제 완료 후)
