...
### 3단계: Google Play 프로텍트 경고 통과 (무시하고 설치)
본 앱은 학생 안내 및 기숙사 릴레이를 위해 `SEND_SMS`(문자 발송) 시스템 권한을 포함하므로, 구글 플레이가 아닌 웹에서 설치할 때 구글 플레이 프로텍트가 빨간색 경고창을 띄웁니다:
1. **"Play 프로텍트에 의해 차단됨"** 또는 **"유해한 앱 차단됨"** 팝업이 뜨면
2. 창 하단의 **`[세부정보 더보기]`** 작은 글씨를 탭합니다.
3. 나타나는 **`[무시하고 설치 (안전하지 않음)]`** 버튼을 클릭하여 설치를 완료합니다.
